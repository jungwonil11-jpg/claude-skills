...
| 33%/67% (단일·다중 무관) | ❌ 매번 즉시 생성 |
| 다중 기능 100% (`1,2 100`) | ❌ 매번 즉시 생성 |
| 장인 모드 | ❌ 빈 파일이라 불필요 |
...
### 학습 제외 필터 — `practice/_filter.md`

강사 원본에 학습 가치 없는 더미 메소드(예: `hello`, `hi`)가 있으면 매번 자동 제외.

**파일 형식** (`practice/_filter.md`):
```markdown
## <파일 경로>
- <메소드명1>
- <메소드명2>
...
**적용 시점 — 템플릿 생성 시만**:
1. 강사 원본 복사
2. `_filter.md` 의 해당 파일 섹션 읽음
3. 명시된 메소드명을 **메소드 어노테이션 + 시그니처 + 본문 + 닫는 `}` 까지 통째로 삭제**
4. 남은 메소드에 빈칸 처리
5. 결과를 `_templates/<기능>_100/` 저장

**적용 X — 백업**:
`.practice-backup/<session-id>/` 는 강사 원본 그대로 (필터 무관). 채점 시 백업 기준이라 hello/hi 같은 더미는 어차피 채점 대상 빈칸 자리 X → 채점 영향 0.

**강사싱크 후 자동 적용**:
1. `/강사싱크` → 강사 코드 업데이트 → hello/hi 다시 들어올 수 있음
2. `/연습 템플릿 재생성 all` → 모든 100% 템플릿 삭제
3. 다음 `/연습 <기능> 100` 호출 시 템플릿 재생성 → `_filter.md` 자동 적용 → hello/hi 빠짐

**주의**:
- 학습 대상 메소드 절대 박지 말 것 (학습 불가)
- 파일 경로 기준. 같은 메소드명 다른 파일에 있어도 영향 X
- `_filter.md` 는 git commit 대상 (다른 컴퓨터 동기화)
...
- **종합 모드**: 다중 기능 100% 의 확장. 도메인의 모든 기능 + VO 파일들까지 한 회차로. 보통 `/도전100` 에서 호출. 별도 워크플로우 — 2-A-종합 섹션에서 다룸.
...
**100% 단일 기능 회차는 템플릿 시스템 우선 사용** ("빈칸 템플릿 시스템" 섹션 참고):
- `practice/_templates/<도메인>-<기능slug>_100/` 존재 → 템플릿 파일들을 src/로 복사 후 종료
- 존재 X → 아래 룰대로 빈칸 처리 + 결과를 `_templates/` 에 저장

**33%/67% 또는 다중 기능 100% 회차**는 매번 아래 룰대로 즉시 빈칸 처리 (템플릿 X):
...
5-파일 묶음 README 와 유사. 학습 대상 파일 경로 전부 명시 + "종합 모드 — 도메인 정복 회차" 강조.
