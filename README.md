...
3. `/연습 채점` → 사용자 답안을 `practice/<회차>/`에 보존 + SCORE.md 작성 + **src/ 원본 자동 복구** (git diff 0)
...
3. **퍼센티지 선택** (이전의 0~3 난이도 대신):
   ...
   - `장인` — 100% 상위. **토큰 빈칸이 아니라 파일 통째로 빈 상태**. 5-파일 묶음 전체를 패키지 선언만 박힌 빈 .java/.xml로 만들어, import부터 클래스 선언부터 SQL까지 사용자가 처음부터 작성. README에 강사 스펙(URL/요청/응답/SQL 동작)만 박힘. 채점은 토큰 단위가 아니라 **컴파일 통과 + 기능 단위 체크리스트**.
4. 5-파일 묶음은 기능 매트릭스가 같이 뜸. **다중 기능 동시 학습**도 지원:
   - 단일: `1 67` (1번 기능을 67%로)
   - 다중: `1,2 67` 또는 `1 2 67` (1번 + 2번을 같이 67%로)
   - 장인: `장인` (모든 구현 기능을 한꺼번에, 토큰 빈칸 없이 빈 파일에서 시작)
