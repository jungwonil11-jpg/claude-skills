...
> **이 스킬들은 제 개인 워크플로우에 맞춰져 있습니다.** 국비학원 자바/Spring 수업(`/강사싱크`·`/공부시작` = 강사 upstream + `study` 브랜치), 노션 학습노트(`/노션메모`·`/노션정리`), 코딩 연습 사이클(`/연습`·`/연습100`·`/힌트`·`/채점` = `src/` 자바 전제), 시니컬 음슴체 페르소나(`/최초셋팅`)가 깔려 있습니다. `/푸쉬`·`/스킬푸쉬`만 워크플로우 독립적입니다.
>
