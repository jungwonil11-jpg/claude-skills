...
4. **`/채점`** 시: 사용자 코드를 `practice/<회차>/`에 답안 보존 + 채점 + src/ 원본 복구
...
> ⚠️ **VO 클래스 예외 (종합 모드 한정)**: VO 파일에서는 **클래스 레벨 Lombok 어노테이션** (`@Getter`, `@Setter`, `@NoArgsConstructor` 등) 과 **필드 선언** (타입 + 필드명) 도 빈칸 처리. VO는 데이터 매핑이 학습 본질이라 어노테이션·필드 살리면 빈칸 0개가 되어 의미 X. 채점 룰은 `/채점` 스킬의 채점 기준 표 (VO 필드명·Lombok 항목) 참고.
...
종합 모드 채점은 `/채점` 스킬이 처리 (`.state` `mode=domain-all`). VO 필드명·Lombok·타입 완전 일치 룰 포함.
...
각 스킬에서 사전 체크 후 거절 메시지 출력. 사용자에게 `/채점` 먼저 안내.
